...
SKILL_REVIEW_PROMPT = (
    ...
    "lessons. PRIMARY output goes to ~/.threadkeeper/lessons.md (shared "
    "across Claude Code / Codex / Gemini / Copilot) via "
    "lesson_append(title, body, summary, source=thread_id). OPTIONAL "
    "additional output: a SKILL.md under ~/.claude/skills/ via "
    "skill_manage(...) — only when Claude-specific frontmatter "
    "auto-triggering adds value over the lesson alone.\n\n"
    + RUBRIC_QUESTIONS + "\n\n"
    "PREFERENCE ORDER (pick the earliest action that fits):\n"
    "  1. PATCH a currently-loaded skill. If the conversation "
    "referenced an existing skill that covers the territory of the new "
    "learning, use skill_manage(action='patch', name=..., "
    "old_string=..., new_string=...) on that one first. New lessons "
    "that overlap existing ones pollute the store — patch beats create.\n"
...
COMBINED_REVIEW_PROMPT = (
    ...
    "/ note as appropriate.\n\n"
    "  **Lessons** — how to handle this class of task. PRIMARY: "
    "lesson_append(title, body, summary, source=thread_id) → "
    "~/.threadkeeper/lessons.md. OPTIONAL Claude-only: skill_manage("
    "...) → ~/.claude/skills/ when frontmatter auto-triggering matters.\n\n"
    + RUBRIC_QUESTIONS + "\n\n"
