...
- **`ingest.py:140` `_backfill_dialog_fts_if_empty` + `:170` join** — keep the helper as a safety net but simplify it to the external-content idiom: when `COUNT(*) FROM dialog_fts == 0` while `dialog_messages` is non-empty, run `INSERT INTO dialog_fts(dialog_fts) VALUES('rebuild')` instead of the per-row `LEFT JOIN … ON f.uuid=d.uuid` + loop (which references the now-removed `uuid` column). `COUNT(*) FROM dialog_fts` (`:153,:192`) — **corrected during implementation:** an unconstrained `COUNT(*)` on an external-content FTS5 table proxies to the content table's rowids (it always equals the `dialog_messages` count, even with an empty index), so every "is the index populated" check counts the `dialog_fts_docsize` shadow table instead.
...
- **`tools/dashboard.py:326`** — **corrected during implementation:** the `dialog_fts=` metric now counts `dialog_fts_docsize` (real index size); a raw `COUNT(*) FROM dialog_fts` would proxy to `dialog_messages` and hide index desync.
