...
rows with no real `SKILL.md` remain explicit orphans. A read-only research
child reads every complete skill and relevant support file, performs current
web research against official docs and comparable public skills, then writes a
bounded `RESEARCH-<pass>-batch-NNN-of-MMM.json` handoff through a
destination-scoped tool. A separate web-free evaluator receives that handoff as
fenced, untrusted data and writes numbered per-skill verdicts to
